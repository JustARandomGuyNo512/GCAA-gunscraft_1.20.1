...
public class AnimationSequence {
    ...
    private int index = 0;
    private final List<KeyframeAnimations.Mark> animations = new ArrayList<>(10);
    private KeyframeAnimations.Mark lasting = null;
    ...
    public AnimationSequence(AnimationDefinition definition, long startTime) {
        animations.add(new KeyframeAnimations.Mark(definition, startTime));
        this.startTime = startTime;
    }

    public AnimationSequence(KeyframeAnimations.Mark mark) {
        animations.add(mark);
        startTime = mark.timeStamp;
    }

    public AnimationSequence append(AnimationDefinition definition) {
        animations.add(new KeyframeAnimations.Mark(definition, 0L));
        return this;
    }

    public AnimationSequence append(KeyframeAnimations.Mark mark) {
        animations.add(mark);
        return this;
    }

    public AnimationSequence finishBuild() {
        for (KeyframeAnimations.Mark mark : animations) {
    ...
    public void apply(HierarchicalModel<?> root) {
        ...
        KeyframeAnimations.Mark mark = animations.get(index);
        if (mark == lasting) {
    ...
    public boolean tick() {
        ...
        if (index < animations.size()) {
            KeyframeAnimations.Mark mark1 = animations.get(index);
